class Lesson{
		...
		static listenNotes(index,l){
			toggleMicrophone(index,l,l.westernLesson[index-1].split(' '));
		}
...
var timer;
var endSwaraFlag;
var remaining_second;
var remaining_second_val;
...
var isAudioContextSupported = function () {
	...
	var detectPitch = function () {
		...
		if (fundalmentalFreq !== -1) {
			...
			if(note.note=='F8'){
				...
				var swarafreq=12*parseInt($('#SCALE').val())+_shruti.indexOf(music_play_note[swaraIndex].slice(0,-1));
				//console.log(music_play_note[swaraIndex],notesArray[swarafreq],note.note,fundalmentalFreq);
		...
		if(swarasCount==1){
				...
				{	var tmp_notes=music_notes;
					...
					for(var i=1;i<tmp_notes.length;i++){
						if(tmp!=tmp_notes[i]){
							if(count>45){
								if(tmp_len>=1 && note_count==0){
									
									if(unique_notes[tmp_len-1][0]!=tmp){
										unique_notes=[];
									}
									else{
										console.log('equalnodes',count)
										var tmp_delay=unique_notes[tmp_len-1][2]
										var tmp_period=unique_notes[tmp_len-1][1]
										unique_notes=[[tmp,tmp_period+count,tmp_delay]];
										flag=1;
										continue;
									}

								}
								unique_notes.push([tmp,count,free_sec]);
								free_sec=0;
								note_count++;
					...
					if(count>45){
						if(tmp_len>=1 && note_count==0){
							if(unique_notes[tmp_len-1][0]!=tmp){
										unique_notes=[[tmp,count,free_sec]];
							}
							else{		console.log('equalnodes',count)
										var tmp_delay=unique_notes[tmp_len-1][2]
										var tmp_period=unique_notes[tmp_len-1][1]
										unique_notes=[[tmp,tmp_period+count,tmp_delay]];
										flag=1;
					...
					console.log(unique_notes,flag,unique_notes[0]);
					if(unique_notes.length>=1){
							for(var i=flag;i<unique_notes.length;i++){
								if(unique_notes[i][0]==music_play_note[swaraIndex])
									tmp_repeatation+=1;
					...
					//continues untill 3 successive correct times...........
			...
			var flag=0;
					if(music_notes.length>45)
						{	var tmp_notes=music_notes;
							...
							var tmp_len=unique_notes.length;
							var d=Date();
							console.log('tmp_len',tmp_len,'micnote[0]',tmp,'total notes',tmp_notes.length,unique_notes)
							...
							if(count>45){
								if(tmp_len>=1 && note_count==0){
									console.log('outside for equalnodes',count)
									if(unique_notes[tmp_len-1][0]!=tmp){
												unique_notes=[[tmp,count,free_sec]];
									}
									else{		//console.log('equalnodes',count)
							...
							console.log('before verification',unique_notes[0],'flag',flag,'swaraIndex',swaraIndex,'left_leaf_node',left_node_mistake,'right_node_mistake',right_node_mistake,'repetation',tmp_repeatation);
							if(unique_notes.length>=1 && tmp_repeatation<repetation){
									for(var i=flag;i<unique_notes.length;i++){
										...
										if(swaraIndex<swarasCount)
											$('#nextnote').text(music_play_note[swaraIndex]);
									}
	...
	var toggleMicrophone = function (index,l,tmp_notes) {
			...
			swaraIndex=0;
			remaining_second_val=2000+swarasCount*2000;
			$('#total_second').text(remaining_second_val);
			$('#nextnote').text(music_play_note[swaraIndex]);
			//Object=l;
			//console.log(music_notes);
			//music_notes=[];
			if (isGetUserMediaSupported()) {
				...
				timer=setInterval(function(){ checkNote(); }, 2500);
				remaining_second = setInterval(function(){
					$('#remaining_second').text(remaining_second_val);
					remaining_second_val--;
				});
